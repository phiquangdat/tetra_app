...
describe('ModulePage', () => {
    ...
    it('renders without crashing and shows loading state', () => {
        vi.spyOn(api, 'fetchModuleById').mockImplementation(() => new Promise(() => {}));
    ...
    it('displays error message if fetch fails', async () => {
        vi.spyOn(api, 'fetchModuleById').mockRejectedValue(new Error('Network error'));
    ...
    it('calls fetchModuleById with the correct id', async () => {
        const fetchSpy = vi.spyOn(api, 'fetchModuleById').mockResolvedValue(mockModule);
