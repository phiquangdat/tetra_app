...
const mockedQuestions = [
    ...
    },
    {
        ...
        options: ['Right to work', 'Right to erasure', 'Right to relocate', 'Right to compensation'],
...
const QuizSummaryPage: React.FC = () => {
    ...
    const correctAnswers = mockedQuestions.filter(q => q.userAnswer === q.correctAnswer).length;
    const incorrectAnswers = mockedQuestions.length - correctAnswers;
    const percentage = Math.round((correctAnswers / mockedQuestions.length) * 100);
    ...
    return (
        <div className="bg-white min-h-screen py-8 px-6">
            ...
            <h1 className="text-2xl font-semibold text-center text-[#231942] mb-1">Quiz Summary</h1>
            <h2 className="text-4xl font-extrabold text-center text-[#14248A] mb-3">Key concepts of data protection</h2>
            <p className="text-center text-gray-700 mb-10">Well done! Here's how you did!</p>

            <div className="flex flex-col md:flex-row items-center justify-center gap-12 mb-12">
                {/* Circular progress bar */}
                <div className="relative w-32 h-32">
                    <svg className="w-full h-full" viewBox="0 0 128 128">
                        <circle cx="64" cy="64" r="45" stroke="#D4C2FC" strokeWidth="10" fill="none" />
                        <circle
                            cx="64"
                            cy="64"
                            r="45"
                    ...
                    <div className="absolute inset-0 flex items-center justify-center text-xl font-bold text-[#14248A]">{percentage}%</div>
                </div>
            ...
            <div className="space-y-6 mb-12">
                {mockedQuestions.map((q, index) => (
                    <div key={q.id} className="bg-[#F5F3F7] border border-gray-300 p-6 rounded-2xl shadow-sm">
                        <h3 className="font-semibold text-[#231942] mb-4">Question {index + 1}: {q.question}</h3>
